...
def migrate_data():
    ...
    with SqliteSession() as sqlite_db, PgSession() as pg_db:
        try:
            print("Beginning migration in a single transaction...")
            ...
            for model, name in [
                ...
                print(f"{name} Count: SQLite={sqlite_count}, PostgreSQL={pg_count}")
            ...
            print(f"Total Amount (Minor Units): SQLite={sqlite_amt}, PostgreSQL={pg_amt}")
            ...
            print(f"Total Refund Amount (Minor Units): SQLite={sqlite_refund}, PostgreSQL={pg_refund}")
            assert sqlite_refund == pg_refund, "Total Refund Amount mismatch"

            print("\nSUCCESS: Migration and validation completed flawlessly.")

        except Exception as e:
            pg_db.rollback()
            print(f"Migration failed and rolled back. Error: {str(e)}")
            sys.exit(1)
