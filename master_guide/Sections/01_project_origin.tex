...
\begin{tikzpicture}[
    node distance=0.8cm,
    step/.style={rectangle, draw=primary!60, thick, fill=bglight, minimum width=4.5cm, minimum height=1cm, rounded corners=4pt, font=\small, text=textdark, drop shadow=black!5},
    arrow/.style={-{Stealth[scale=1.1]}, thick, draw=primary!60},
    label/.style={font=\scriptsize\itshape, text=textmuted, align=center}
]

    \node[step] (cust) {Customer Attempts Payment};
    \node[step, below=of cust] (fail) {Payment Fails};
    \node[step, below=of fail] (ingest) {RecoverAI Ingests Transaction};
    \node[step, below=of ingest] (ml) {ML Model Predicts Recovery Probability};
    \node[step, below=of ml] (llm) {LLM Agent Diagnoses Failure};
    \node[step, below=of llm] (policy) {Policy Engine Evaluates Safety};
    \node[step, below=of policy] (guard) {ExecutionGuard Validates Invariants};
    \node[step, below=of guard] (gateway) {Gateway Executes Recovery Action};
    \node[step, below=of gateway] (verify) {Verification \& Final State};
