...
\begin{tikzpicture}[
    node distance=1.2cm,
    timeline/.style={draw=primary, thick, -{Stealth}, align=center},
    batchnode/.style={circle, draw=primary, thick, fill=bglight, minimum size=1.2cm, font=\bfseries\color{primary}, align=center},
...
\begin{tikzpicture}[
    ...
    \node[stepbox, below=of b5] (b52) {\textbf{Batch 5.2:} Replaced FastAPI \code{BackgroundTasks} with Celery + Redis for durable orchestrations.};
    \node[stepbox, below=of b52] (b53) {\textbf{Batch 5.3:} Migrated webhook processing to durable Celery background tasks.};
    \node[stepbox, below=of b53] (b531) {\textbf{Batch 5.3.1 (Current):} Implemented bounded webhook retry limits to prevent poison-pill queue starvation.};
\end{tikzpicture}
\end{center}
