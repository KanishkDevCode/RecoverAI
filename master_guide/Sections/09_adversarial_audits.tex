...
\begin{tikzpicture}[
    node distance=1cm,
    step/.style={rectangle, draw=primary, thick, fill=bglight, minimum width=4cm, minimum height=0.8cm, rounded corners=4pt, font=\small, drop shadow=black!5},
    arrow/.style={-{Stealth[scale=1.1]}, thick, draw=textdark}
]

    \node[step] (fail) {Webhook Task Fails};
    \node[step, below=of fail] (inc) {\code{retry\_count += 1} \& \code{last\_attempt\_at = NOW()}};
